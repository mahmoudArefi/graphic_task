...
void Pharmacy::draw() {
    prObj.drawPR(pH, pW, pD, myTextureObj.pharmacyTexture);
   ...
   float labelH = pH*0.15 , topPartH = pH*0.22 , sidePartW = pW*0.4 ;
	...
	polygonObj.drawPolygon(labelH, pW , myTextureObj.pharmacyLabel  ) ;
	glTranslated(0,-(labelH+topPartH)*0.5,0);
	polygonObj.drawPolygon(topPartH,	pW , myTextureObj.pharmacy_wall1 , 0.5  ) ;
	glTranslated((pW-sidePartW)*0.5, topPartH - (pH-(labelH+topPartH)) , 0);
	polygonObj.drawPolygon(pH - ( labelH+topPartH) ,	sidePartW  , myTextureObj.pharmacy_wall1 , 0.5  ) ;
...
void Pharmacy::drawWallBetweenWareHouseAndPharmacy(){
	...
	prObj.drawParallelRectangleWithSameTexture(pH , pW*0.8 , 1,myTextureObj.pharmacy_wall2,true) ;
}